...
def orders_activations(request):
    ...
    orders_all = Orders.objects.filter(id_sim_id__isnull=False).filter(activation_date__gte=today).order_by('activation_date').prefetch_related('product__choices')
    ...
    # orders_df['days'] = pd.to_timedelta(orders_df['days'], unit='d')
    ...
    if request.method == 'GET':
        ...
        activGoing_1 = request.GET.get('activGoing_1')
        activGoing_2 = request.GET.get('activGoing_2')
        activReturn_1 = request.GET.get('activReturn_1')
        activReturn_2 = request.GET.get('activReturn_2')
        oper_f = request.GET.get('oper')
        ord_st_f = request.GET.get('ord_st')
